...
from src.platform.imports import require_platform_sdk
...
class PlatformSDKAdapter:
    ...
    The official SDK is required only when an SDK operation is actually used.
    """

    def __init__(self, **sdk_kwargs):
        sdk_class = require_platform_sdk()

        config = dict(SDK_CONFIG)
        config.update(sdk_kwargs)

        self.sdk = sdk_class(**config)
