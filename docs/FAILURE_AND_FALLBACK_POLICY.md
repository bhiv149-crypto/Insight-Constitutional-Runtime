...
| live quantum hardware SDK not installed | Runtime reports `CREDENTIALS_REQUIRED` / `UNAVAILABLE`; falls back to live simulator | `FALLBACK` |
...
| live quantum hardware SDK / API keys not available | Infrastructure / Dhiraj | Execution remains `QUANTUM_SIMULATED` on live runtime |
